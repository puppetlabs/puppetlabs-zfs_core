...
def zpool_setup(agent, o = {})
  o = OPTS.merge(o)
  on agent, "mkdir -p #{o[:poolpath]}/mnt||:"
  on agent, "#{os_mkfile_command(agent)} 100m #{o[:poolpath]}/dsk1 #{o[:poolpath]}/dsk2 #{o[:poolpath]}/dsk3 #{o[:poolpath]}/dsk5 ||:"
  on agent, "#{os_mkfile_command(agent)} 50m #{o[:poolpath]}/dsk4 ||:"
end
